...
class AirQualityProvider with ChangeNotifier {
  ...
  AirQuality _items = AirQuality(
    ...
    data: Data(
      ...
      iaqi: Iaqi(
          ...
          w: Co(v: 0.625)),
      time: Time(
        s: DateTime(2020, 06, 27, 21, 00, 00),
        tz: "+05:30",
        v: 1593291600,
      ),
      forecast: Forecast(
  ...
  Future<void> fetchAndSetData() async {
    final url = 'https://api.waqi.info/feed/shanghai/?token=$apiKey';
    try {
      // _isLoading = true;
      final response = await get(url);
      // print(response.body);
      _items = AirQuality.fromJson(json.decode(response.body));
      print(_items.data.aqi);
      _isLoading = false;
      notifyListeners();
      return;
    } catch (e) {}
